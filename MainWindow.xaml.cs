...
namespace Microsoft.Kinect.InteractiveKinectFusionExplorer
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged, IDisposable
    {
        ...
        /// <summary>
        /// Stop integrating pixels into the volume
        /// </summary>
        private bool stopIntegration = false;

        /// <summary>
        /// Toggle interaction mode, toggled by "Interaction mode" button
        /// </summary>
        private bool interactionMode = false;

        /// <summary>
        /// All the checkBox values used for the new interaction mode
        /// </summary>
        private bool showCamInfo = false;
        private bool showCoords = false;
        private bool showFPS = true;
        private bool showTriangleCount = true;
        private bool showGrid = false;
        private bool showCube = false;
        private bool reduceVertices = true;
        private bool segmentMesh = true;
        private bool grabCarry = true;
        private bool segmentWall = false;
        private bool cameraControls = false;
        private bool interpolateColor = false;
        private bool highlightObject = false;

        /// <summary>
        /// Flag is set iff the left mouse button is being clicked
        /// </summary>
        private bool mouseIsDown = false;

        /// <summary>
        /// All the slider values used for the new interaction mode
        /// </summary>
        private int meshQuality = 3;
        private float wallSensitivity = 0.20f;
        private float blockSize = 0.30f;
        private float grabDistance = 0.50f;
        private double redValue = 128;
        private double greenValue = 128;
        private double blueValue = 128;

        /// <summary>
        /// Temporary storage for the currently selected object
        /// </summary>
        private Material selectedObjectStoredMaterial;
        private GeometryModel3D SelectedObject;
        private Dictionary<GeometryModel3D, Transform3D> SelectedObjectStoredTransforms = new Dictionary<GeometryModel3D, Transform3D>();
        //private Matrix3D previousCameraRotationMatrix = new Matrix3D();        

        #endregion
        #region mostly unchanged kinect fusion explorer
        /// <summary>
        ...
        /// <summary>
        /// Binding property to check box "Volume Graphics"
        /// </summary>
        ...
        /// <summary>
        /// Binding property to check box "Use Camera Pose Finder"
        /// </summary>
        ...
        private void StartStreams(DepthImageFormat depthFormat, ColorImageFormat colorFormat)
        {
            try
            {
                ...
                // Here we setup two callbacks - one process depth just for camera tracking and runs at 30fps
                // The other processes when both the depth and color are ready for depth and color integration,
                // but this will only run at 12fps when using high res color.
            ...
            catch (InvalidOperationException)
            {
                ...
                this.nearModeCheckBox.IsChecked = false;
        ...
        private void Process()
        {
            ...
            if (this.stopIntegration)
            {
                ...
                if (0 == this.trackingErrorCount)
                {
                    ...
                    {
                        // Raycast and render
                        if (!this.interactionMode) this.RenderReconstruction();
                    }
            ...
            //ADDED !this.stopIntegration
            if (!this.stopIntegration && null != this.volume && !this.savingMesh)
            {
                try
                {
                    // Check if camera pose finder is available
        ...
        private bool TrackCameraAlignPointClouds(ref bool calculateDeltaFrame, ref Matrix4 calculatedCameraPose)
        {
            ...
            {
                bool failed = Helper.CameraTransformFailed(
                    initialPose,
        ...
        private void TrackCamera()
        {
            ...
            if (!this.trackingFailed && 0 != this.successfulFrameCount)
            {
        ...
        private void ProcessColorForCameraPoseFinder()
        {
            ...
            if (this.depthPixelCount != this.colorPixelCount)
            {
                if (this.colorPixelCount > this.depthPixelCount)
                {
                    // Down-sample
                    int factor = this.colorWidth / this.resampledColorFrame.Width;
                    this.DownsampleColorFrameNearestNeighbor(this.resampledColorFrame, factor);
                }
                else
                {
                    // Up-sample
                    int factor = this.resampledColorFrame.Width / this.colorWidth;
                    this.UpsampleColorFrameNearestNeighbor(this.resampledColorFrame, factor);
                }
            }
            else
            {
                this.CopyColorImageFrame(this.resampledColorFrame);
            }
        }
        ...
        private bool IntegrateData()
        {
            ...
            //ADDED 4) integration stopped
            bool integrateData = !this.trackingFailed && ((!this.cameraPoseFinderAvailable && !this.PauseIntegration && !this.StopIntegration)
                || (this.cameraPoseFinderAvailable && !(this.trackingHasFailedPreviously && this.successfulFrameCount < MinSuccessfulTrackingFramesForCameraPoseFinderAfterFailure)));
        ...
        private void CreateMeshButtonClick(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                lock (this.volumeLock)
                {
                    ...
                    mesh = this.volume.CalculateMesh(2);
                    Console.WriteLine("Viewportinfos save: Count: " + GraphicsViewport);
                }
        ...
        private void CopyColorImageFrame(FusionColorImageFrame dest)
        {
            ...
            Buffer.BlockCopy(this.colorImagePixels, 0, this.resampledColorImagePixels, 0, this.colorPixelCount * sizeof(int));
        ...
        /// <summary>
        /// Down sample color pixels with nearest neighbor
        /// </summary>
        /// <param name="dest">The destination color image.</param>
        /// <param name="factor">The down sample factor (2=x/2,y/2, 4=x/4,y/4, 8=x/8,y/8, 16=x/16,y/16).</param>
        private unsafe void DownsampleColorFrameNearestNeighbor(FusionColorImageFrame dest, int factor)
        {
            if (null == dest || null == this.colorImagePixels || null == this.resampledColorImagePixels)
            {
            ...
            int downsampleWidth = this.colorWidth / factor;
            int downsampleHeight = this.colorHeight / factor;
            ...
            fixed (byte* ptrColorPixels = this.colorImagePixels)
            {
                ...
                Parallel.For(
                    ...
                    y =>
                    {
                        ...
                        int sourceColorIndex = y * this.colorWidth * factor;

                        for (int x = 0; x < downsampleWidth; ++x, ++destIndex, sourceColorIndex += factor)
                        {
                            // Copy color pixel
        ...
        /// <summary>
        /// Down sample depth pixels with nearest neighbor
        /// </summary>
        ...
        private unsafe void DownsampleDepthFrameNearestNeighbor(FusionFloatImageFrame dest, int factor)
        {
            ...
            if (this.mirrorDepth)
            {
                fixed (DepthImagePixel* rawDepthPixelPtr = this.depthImagePixels)
                {
                    DepthImagePixel* rawDepthPixels = (DepthImagePixel*)rawDepthPixelPtr;
                    ...
                    Parallel.For(
                        ...
                        y =>
                        {
                            ...
                            {
                                // Copy depth value
                                this.downsampledDepthImagePixels[destIndex] = (float)rawDepthPixels[sourceIndex].Depth * 0.001f;
                            }
                        });
                }
            }
                ...
                fixed (DepthImagePixel* rawDepthPixelPtr = this.depthImagePixels)
                {
                    ...
                    Parallel.For(
                        ...
                        y =>
                        {
                            ...
                            {
                                // Copy depth value
                                this.downsampledDepthImagePixels[flippedDestIndex] = (float)rawDepthPixels[sourceIndex].Depth * 0.001f;
                            }
                        });
                }
            }
        ...
        /// <summary>
        /// Up sample input color frame with nearest neighbor - replicates pixels
        /// </summary>
        ...
        private unsafe void UpsampleColorFrameNearestNeighbor(FusionColorImageFrame dest, int factor)
        {
            if (null == dest || null == this.colorImagePixels || null == this.resampledColorImagePixels)
            {
                throw new ArgumentException("inputs null");
            ...
            int upsampleWidth = this.colorWidth * factor;
            int upsampleHeight = this.colorHeight * factor;

            if (dest.Width != upsampleWidth || dest.Height != upsampleHeight)
            {
                throw new ArgumentException("dest != upsampled image size");
            }
            ...
            fixed (byte* ptrColorPixels = this.colorImagePixels)
            {
                ...
                Parallel.For(
                    ...
                    y =>
                    {
                        ...
                        int sourceColorIndex = y * this.colorWidth;

                        for (int x = 0; x < upsampleWidth; ++x, ++sourceColorIndex)
                        {
                            ...
                            {
                                // Copy color pixel
                                this.resampledColorImagePixels[destIndex] = color;
                            }
                        }
                    });
            }
            ...
            int rowByteSize = upsampleWidth * sizeOfInt;
            ...
            for (int y = 0; y < this.colorHeight; ++y)
            {
                ...
                for (int r = 1; r < factor; ++r)
                {
                    ...
                    Buffer.BlockCopy(
                        this.resampledColorImagePixels, srcRowIndex * sizeOfInt, this.resampledColorImagePixels, index * sizeOfInt, rowByteSize);
                }
            }
                ...
                Parallel.For(
                    ...
                    y =>
                    {
                        ...
                        for (int x = 0; x < upsampleWidth; ++x, ++sourceColorIndex)
                        {
                            ...
                            {
                                // Copy color pixel
                                this.deltaFromReferenceFramePixelsArgb[destIndex] = color;
                            }
                        }
                    });
            }
        ...
        #endregion
        #region added dependency methods
        /// <summary>
        /// Binding property to slider "Grab distance"
        /// </summary>
        public double GrabDistance
        {
            get
            {
                return (double)this.grabDistance;
            }

            set
            {
                this.grabDistance = (float)value;
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("GrabDistance"));
                }
            }
        }

        /// <summary>
        /// Binding property to slider "Highlight object"
        /// </summary>
        public bool HighlightObject
        {
            ...
            set
            {
                this.highlightObject = value;
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("HighlightObject"));
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Interpolate Color"
        /// </summary>
        public bool InterpolateColor
        {
            ...
            set
            {
                ...
                if (this.interpolateColor && !CaptureColor)
                    CaptureColor = true;
        ...
        /// <summary>
        /// Binding property to slider "Wall"
        /// </summary>
        public double WallSensitivity
        {
            get
            {
                return (double)this.wallSensitivity;
            }

            set
            {
                this.wallSensitivity = (float)value;
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("WallSensitivity"));
                }
            }
        }

        /// <summary>
        /// Binding property to slider "Block size"
        /// </summary>
        public double BlockSize
        {
            get
            {
                return (double)this.blockSize;
            }

            set
            {
                this.blockSize = (float)value;
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("BlockSize"));
                }
            }
        }

        /// <summary>
        /// Binding property to slider "Quality"
        /// </summary>
        public double MeshQuality
        {
            get
            {
                return this.meshQuality;
            }

            set
            {
                this.meshQuality = (int)value;
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("MeshQuality"));
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Segment wall"
        /// </summary>
        public bool SegmentWall
        {
            get
            {
                return this.segmentWall;
            }

            set
            {

                this.segmentWall = value;
                if (this.segmentWall && !SegmentMesh)
                    SegmentMesh = true;

                if (this.segmentWall)
                    wallSlider.Visibility = Visibility.Visible;
                else
                    wallSlider.Visibility = Visibility.Collapsed;

                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("SegmentWall"));
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Grab Carry"
        /// </summary>
        public bool GrabCarry
        {
            get
            {
                return this.grabCarry;
            }
            ...
            set
            {
                if (!this.mouseIsDown)
                {
                    this.grabCarry = value;
                    if (null != this.PropertyChanged)
                    {
                        this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("MouseMode"));
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Binding property to check box "Camera Controls"
        /// </summary>
        public bool CameraControls
        {
            ...
            set
            {
                ...
                {
                    GraphicsViewport.IsManipulationEnabled = false;
                    GraphicsViewport.IsZoomEnabled = false;
                    GraphicsViewport.IsMoveEnabled = false;
                    GraphicsViewport.IsPanEnabled = false;
                    GraphicsViewport.IsRotationEnabled = false;
                }
                else
                {
                    GraphicsViewport.IsManipulationEnabled = true;
                    GraphicsViewport.IsZoomEnabled = true;
                    GraphicsViewport.IsMoveEnabled = true;
                    GraphicsViewport.IsPanEnabled = true;
                    GraphicsViewport.IsRotationEnabled = true;
                }
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("CameraControls"));
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Reduce Vertices"
        /// </summary>
        public bool ReduceVertices
        {
            get
            {
                return this.reduceVertices;
            }

            set
            {
                this.reduceVertices = value;
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("ReduceVertices"));
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Segment mesh"
        /// </summary>
        public bool SegmentMesh
        {
            get
            {
                return this.segmentMesh;
            }

            set
            {
                this.segmentMesh = value;
                if (!this.segmentMesh && SegmentWall)
                    SegmentWall = false;

                if (this.segmentMesh)
                    blockSlider.Visibility = Visibility.Visible;
                else
                    blockSlider.Visibility = Visibility.Collapsed;

                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("SegmentMesh"));
                }
            }
        }

        /// <summary>
        /// Binding property to slider "Red"
        /// </summary>
        public double RedValue
        {
            get
            {
                return (double)this.redValue;
            }

            set
            {
                this.redValue = (int)(value + 0.5);
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("RedValue"));
                }
            }
        }

        /// <summary>
        /// Binding property to slider "Blue"
        /// </summary>
        public double BlueValue
        {
            get
            {
                return (double)this.blueValue;
            }

            set
            {
                this.blueValue = (int)(value + 0.5);
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("BlueValue"));
                }
            }
        }

        /// <summary>
        /// Binding property to slider "Green"
        /// </summary>
        public double GreenValue
        {
            get
            {
                return (double)this.greenValue;
            }

            set
            {
                this.greenValue = (int)(value + 0.5);
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("GreenValue"));
                }
            }
        }
        ...
        /// <summary>
        /// Binding property to check box "Stop Integration"
        /// </summary>
        public bool StopIntegration
        {
            get
            {
                return this.stopIntegration;
            }

            set
            {
                this.stopIntegration = value;
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("StopIntegration"));
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Show Camera Info"
        /// </summary>
        public bool ShowCamInfo
        {
            get
            {
                return this.showCamInfo;
            }

            set
            {
                this.showCamInfo = value;

                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("ShowCamInfo"));
                }

                if (this.showCamInfo)
                {
                    GraphicsViewport.ShowCameraInfo = true;
                }
                else
                {
                    GraphicsViewport.ShowCameraInfo = false;
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Show Grid"
        /// </summary>
        public bool ShowGrid
        {
            ...
            set
            {
                ...
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("ShowGrid"));
                }
        ...
        /// <summary>
        /// Binding property to check box "Show Cube"
        /// </summary>
        public bool ShowCube
        {
            ...
            set
            {
                ...
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("ShowCube"));
                }

                if (this.showCube)
                {
                    GraphicsViewport.ShowViewCube = true;
                }
                else
                {
                    GraphicsViewport.ShowViewCube = false;
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Show Coords"
        /// </summary>
        public bool ShowCoords
        {
            ...
            set
            {
                this.showCoords = value;

                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("ShowCoords"));
                }

                if (this.showCoords)
                {
                    GraphicsViewport.ShowCoordinateSystem = true;
                }
                else
                {
                    GraphicsViewport.ShowCoordinateSystem = false;
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Show Triangle Count"
        /// </summary>
        public bool ShowTriangleCount
        {
            get
            {
                return this.showTriangleCount;
            }
            ...
            set
            {
                ...
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("ShowTriangleCount"));
                }

                if (this.showTriangleCount)
                {
                    GraphicsViewport.ShowTriangleCountInfo = true;
                }
                else
                {
                    GraphicsViewport.ShowTriangleCountInfo = false;
                }
            }
        }

        /// <summary>
        /// Binding property to check box "Show FPS"
        /// </summary>
        public bool ShowFPS
        {
            get
            ...
            set
            {
                ...
                if (null != this.PropertyChanged)
                {
                    this.PropertyChanged.Invoke(this, new PropertyChangedEventArgs("ShowFPS"));
                }

                if (this.showFPS)
                {
                    GraphicsViewport.ShowFrameRate = true;
                }
                else
                {
                    GraphicsViewport.ShowFrameRate = false;
                }
            }
        }
        #endregion
        #region added button click methods

        /// <summary>
        /// Handler for interaction mode toggle event
        /// Loads and shows the interaction mode and stops all volume integration,
        /// only camera tracking is still active
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        private void InteractionModeButtonClick(object sender, RoutedEventArgs e)
        {
            if (GraphicsViewport.Visibility != Visibility.Visible)
            {
                //this stops capture mode
                ...
                lock (this.volumeLock)
                {
                    ...
                    //save current volume in a readable format
                    mesh = this.volume.CalculateMesh(meshQuality);
                }
                this.savingMesh = false;

                try
                {
                    GraphicsViewport.DataContext = new MainViewModel(mesh, reduceVertices, segmentMesh, segmentWall,
                        blockSize, wallSensitivity, interpolateColor, captureColor);
                }
                catch (Exception ef)
                {
                    this.ShowStatusMessage(ef.Message +" while trying to enter InteractionMode (maybe no vertices?)");
                    interactionMode = false;
                    stopIntegration = false;
                    return;
                }

                //hide capture mode gui
                this.shadedSurfaceImage.Visibility = Visibility.Hidden;
                OptionsGrid.Visibility = Visibility.Collapsed;

                //set up the Viewport
                GraphicsViewport.Camera.Position = new Point3D(0, 0, 0);
                GraphicsViewport.Camera.LookDirection = new Vector3D(0, 0, -1);
                GraphicsViewport.Camera.UpDirection = new Vector3D(0, 1, 0);
                GraphicsViewport.ShowCoordinateSystem = this.showCoords;
                GraphicsViewport.Background = new SolidColorBrush(Colors.Black);
                GraphicsViewport.ShowFrameRate = this.showFPS;
                GraphicsViewport.ShowTriangleCountInfo = this.showTriangleCount;
                GraphicsViewport.ShowViewCube = this.showCube;
                GraphicsViewport.IsManipulationEnabled = false;
                GraphicsViewport.IsMoveEnabled = false;
                GraphicsViewport.IsPanEnabled = false;
                GraphicsViewport.IsRotationEnabled = false;
                GraphicsViewport.IsZoomEnabled = false;
                helixGrid.Visible = this.showGrid;

                GraphicsViewport.IsHeadLightEnabled = true;

                //show interaction mode gui
                GraphicsViewport.Visibility = Visibility.Visible;
                InteractionModeOptionsGrid.Visibility = Visibility.Visible;
            ...
            {
                //hide interaction mode gui
                GraphicsViewport.Visibility = Visibility.Hidden;
                InteractionModeOptionsGrid.Visibility = Visibility.Hidden;
                //unload model
                GraphicsViewport.DataContext = null;

                //show capture mode gui
                this.shadedSurfaceImage.Visibility = Visibility.Visible;
                OptionsGrid.Visibility = Visibility.Visible;
                //depthFloatImageViewbox.Visibility = Visibility.Visible;
                //deltaFromReferenceImageViewbox.Visibility = Visibility.Visible;
        ...
        /// <summary>
        /// Handler for interaction mode fullscreen toggle event
        /// Hides all GUI except the model viewport and enters
        /// fullscreen mode
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        private void IMFullscreenClick(object sender, RoutedEventArgs e)
        {
            //hide interaction mode gui
            InteractionModeOptionsGrid.Visibility = Visibility.Collapsed;
            OptionsGrid.Visibility = Visibility.Collapsed;
            depthFloatImageViewbox.Visibility = Visibility.Collapsed;
            deltaFromReferenceImageViewbox.Visibility = Visibility.Collapsed;

            //change model viewport size to fit fullscreen
            GraphicsViewport.Height = this.RenderSize.Height;
            double tmpRatio = depthWidth / depthHeight;
            GraphicsViewport.Width = this.RenderSize.Height * tmpRatio;

            //register key event handler to allow escaping fullscreen mode
        ...
        void OnKeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.Key == Key.Escape)
            {
                ...
                //readjust model viewport size to fit screen
                GraphicsViewport.Height = CompositeHost.RenderSize.Height;
                double tmpRatio = depthWidth / depthHeight;
                GraphicsViewport.Width = CompositeHost.RenderSize.Height * tmpRatio;
        ...
        /// <summary>
        /// Handler for reset viewport camera click events
        /// Resets the HelixViewport3D Camera object to the initital settings
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        private void ResetViewportCameraClick(object sender, RoutedEventArgs e)
        {
            GraphicsViewport.Camera.Position = new Point3D(0, 0, 0);
            GraphicsViewport.Camera.LookDirection = new Vector3D(0, 0, -1);
            GraphicsViewport.Camera.UpDirection = new Vector3D(0, 1, 0);
        ...
        /// <summary>
        /// Handler for create mesh click events in interaction mode
        /// Exports a .ply file containing the mesh currently displayed in HelixViewport
        /// ALSO INCLUDES COLORS IF EXISTING
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        private void CreateInteractionMeshButtonClick(object sender, RoutedEventArgs e)
        {
            DateTime beginning = DateTime.UtcNow;
            try
            {
                ...
                lock (this.volumeLock)
                {
                    ...
                    //get custom mesh including all meshes in the viewport and their color values
                    mesh = (GraphicsViewport.DataContext as MainViewModel).GetCustomMesh();
                }
                ...
                //export PLY
                Win32.SaveFileDialog dialog = new Win32.SaveFileDialog();
                dialog.FileName = "ExportedMesh.ply";
                dialog.Filter = "PLY Mesh Files|*.ply|All Files|*.*";
                if (true == dialog.ShowDialog())
                {
                    using (StreamWriter writer = new StreamWriter(dialog.FileName))
                    {
                        Helper.SaveAsciiPlyMeshIM(mesh, writer, false, this.colorCaptured);
                    }
            ...
            Viewport3DHelper.Export(GraphicsViewport.Viewport, d.FileName);*/
        }

        /// <summary>
        /// Handler for apply material click event
        /// applies a single diffuse material to every mesh in the HelixViewport
        /// every other material will be overwritten
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        private void ApplyMaterialClick(object sender, RoutedEventArgs e)
        {
            var r = System.Convert.ToByte(redValue);
            var g = System.Convert.ToByte(greenValue);
            var b = System.Convert.ToByte(blueValue);

            Color tmpColor = Color.FromRgb(r, g, b);
            (GraphicsViewport.DataContext as MainViewModel).ChangeMaterial(tmpColor);
        }
        #endregion
        #region added hit testing methods

        /// <summary>
        /// Handler for left mouse button click event
        /// Only relevant for interaction mode
        /// Handles hit testing at mouse position and subsequent object transformation 
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        public void LeftMouseButtonDown_HitTest(object sender, MouseButtonEventArgs args)
        {
            //dont repeat while mouse button is pressed
            if (!mouseIsDown)
            {
                //get current mouse position relative to HelixViewport
                Point p = args.GetPosition(GraphicsViewport);

                Point3D hitP;

                //hit test at current mouse position
                GeometryModel3D source = FindHitSource(p, out hitP);

                //if wall segmentation is activated, then a wall hit is not processed here
                if (source != null && !(GraphicsViewport.DataContext as MainViewModel).isWallModel(source))
                {
                    Console.WriteLine("standard hit" + hitP);

                    mouseIsDown = true;

                    //temporarily store object material
                    selectedObjectStoredMaterial = source.Material;

                    if (this.highlightObject)
                    {
                        //apply green material to highlight object
                        var greenMaterial = MaterialHelper.CreateMaterial(Colors.Green);
                        source.Material = greenMaterial;
                        source.BackMaterial = greenMaterial;
                    }

                    //set selected object
                    SelectedObject = source;

                    /* temporarily store object transformation for possible automatic retransformation
                     * iff user tries to store selected object in empty space
                     */
                    if (SelectedObjectStoredTransforms.ContainsKey(SelectedObject))
                        SelectedObjectStoredTransforms[SelectedObject] = SelectedObject.Transform;
                    else SelectedObjectStoredTransforms.Add(SelectedObject, SelectedObject.Transform);

                    //register mouse move eventhandler to attach object to mouse cursor
                    CompositeHost.MouseMove += OnMouseMove;

                    //TransformToCamera(SelectedObject);

                    //register cameratransformationchanged eventhandler to update object position as camera moves
                    virtualCamera.CameraTransformationChanged += InteractionModeCameraTransformChanged;
                }
                else
                {
                    var ray = Viewport3DHelper.Point2DtoRay3D(GraphicsViewport.Viewport, p);
                    if (ray != null)
                    {
            ...
            {
                //if grab carry is not activated, handle object to hitpoint transformation here

                //restore selected object material
                SelectedObject.Material = selectedObjectStoredMaterial;
                SelectedObject.BackMaterial = selectedObjectStoredMaterial;

                //unregister events to detach object from cursor
                virtualCamera.CameraTransformationChanged -= InteractionModeCameraTransformChanged;
                CompositeHost.MouseMove -= OnMouseMove;

                //pressing SPACE while clicking will return the selected object to its initial location
                bool backToInit = (Keyboard.IsKeyDown(Key.Space));
                Point p = args.GetPosition(GraphicsViewport);
                Point3D hitP;
                GeometryModel3D source = FindHitSource(p, out hitP);
                if (source != null)
                {
                    if (backToInit)
                        SelectedObjectStoredTransforms[SelectedObject] = new TranslateTransform3D(0, 0, 0);
                    else
                        TransformFromMouseToHitpoint(SelectedObject, hitP);
                    //TransformFromCameraToHitpoint(SelectedObject, hitP);
                }
                else
                {
                    //if empty space is selected, restore selected object to previous position
                    SelectedObject.Transform = SelectedObjectStoredTransforms[SelectedObject];
                }

                //unset selected object so that a new object can be selected
                selectedObjectStoredMaterial = null;
                SelectedObject = null;
                mouseIsDown = false;
            }
        }
        ...
        /// <summary>
        /// Transforms a model from the mouse cursor to a hitpoint
        /// </summary>
        /// <param name="model">The geometry model</param>
        /// <param name="hitPoint">The hitpoint</param>
        private void TransformFromMouseToHitpoint(GeometryModel3D model, Point3D hitPoint)
        {
            Transform3DGroup transform = new Transform3DGroup();

            //calculate center point of model for further transformations
            Point3D centerPoint = GetCenterPoint(model);

            //calculate distance point of model for further transformations
            Point3D distancePoint = GetPointOfGreatestDistance(model);

            //offset ensures that object will not be placed too far inside another object, but rather in front of it
            Point3D offset = new Point3D();
            offset.X = centerPoint.X - distancePoint.X;
            offset.Y = centerPoint.Y - distancePoint.Y;
            offset.Z = centerPoint.Z - distancePoint.Z;

            //raycast from mouse position into viewport, we get the position and direction of the cursor in world space
            Point p = Mouse.GetPosition(GraphicsViewport);
            var pointInMiddle = new Point3D();
            var pointInFar = new Point3D();
            Viewport3DHelper.Point2DtoPoint3D(GraphicsViewport.Viewport, p, out pointInMiddle, out pointInFar);

            //calculate selected object position based on grab distance and mouse position
            Point3D newPoint = new Point3D();
            newPoint.X = pointInMiddle.X + (this.grabDistance / 10) * (pointInFar.X - pointInMiddle.X);
            newPoint.Y = pointInMiddle.Y + (this.grabDistance / 10) * (pointInFar.Y - pointInMiddle.Y);
            newPoint.Z = pointInMiddle.Z + (this.grabDistance / 10) * (pointInFar.Z - pointInMiddle.Z);

            TranslateTransform3D translateFromMouseToOrigin =
                    new TranslateTransform3D(newPoint.X * -1, newPoint.Y * -1, newPoint.Z * -1);

            TranslateTransform3D translateFromOriginToHitPoint =
                new TranslateTransform3D(hitPoint.X, hitPoint.Y, hitPoint.Z);

            TranslateTransform3D translateFromHitPointToEdge =
                new TranslateTransform3D(offset.X, offset.Y, offset.Z);

            //dont forget previous transformations
            transform.Children.Add(model.Transform);

            transform.Children.Add(translateFromMouseToOrigin);
            transform.Children.Add(translateFromOriginToHitPoint);
            transform.Children.Add(translateFromHitPointToEdge);

            model.Transform = transform;
        }

        /*void TransformToCamera(GeometryModel3D model)
        {
            Transform3DGroup transform = new Transform3DGroup();

            Point3D centerPoint = GetCenterPoint(model);

            //Vector3D tmp = new Vector3D(point.X, point.Y, point.Z);
            Point3D tmpPoint = GraphicsViewport.Camera.Transform
                .Transform(new Point3D(0,0,-0.6));

            Point3D cameraPoint = new Point3D(virtualCamera.WorldToCameraMatrix3D.OffsetX,
                virtualCamera.WorldToCameraMatrix3D.OffsetY, virtualCamera.WorldToCameraMatrix3D.OffsetZ);
            Point3D differencePoint = new Point3D();
            differencePoint.X = tmpPoint.X - cameraPoint.X;
            differencePoint.Y = tmpPoint.Y - cameraPoint.Y;
            differencePoint.Z = tmpPoint.Z - cameraPoint.Z;

            TranslateTransform3D translateToOrigin =
                    new TranslateTransform3D((centerPoint.X * -1) + differencePoint.X, (centerPoint.Y * -1) + differencePoint.Y, (centerPoint.Z * -1) + differencePoint.Z);// - 0.6);

            TranslateTransform3D translateFromOriginToCamera =
                new TranslateTransform3D(virtualCamera.WorldToCameraMatrix3D.OffsetX,
                    virtualCamera.WorldToCameraMatrix3D.OffsetY,
                    virtualCamera.WorldToCameraMatrix3D.OffsetZ);

            Matrix3D cameraRotationMatrix = GetCurrentCameraMatrix();

            MatrixTransform3D rotateWithCamera = new MatrixTransform3D(cameraRotationMatrix);

            transform.Children.Add(model.Transform);
            transform.Children.Add(translateToOrigin);
            transform.Children.Add(rotateWithCamera);
            transform.Children.Add(translateFromOriginToCamera);
            

            model.Transform = transform;
        }
        */
        /*void TransformFromCameraToHitpoint(GeometryModel3D model, Point3D hitPoint)
        {
            Transform3DGroup transform = new Transform3DGroup();
            ...
            offset.Z = centerPoint.Z - distancePoint.Z;
            TranslateTransform3D translateToOrigin =
                    new TranslateTransform3D(centerPoint.X * -1, centerPoint.Y * -1, centerPoint.Z * -1);
            ...
            TranslateTransform3D translateFromCameraToOrigin =
                new TranslateTransform3D((virtualCamera.WorldToCameraMatrix3D.OffsetX * -1) - differencePoint.X,
                    (virtualCamera.WorldToCameraMatrix3D.OffsetY * -1) - differencePoint.Y,
                    (virtualCamera.WorldToCameraMatrix3D.OffsetZ * -1) - differencePoint.Z); //+ 0.6);
            ...
            MatrixTransform3D rotateAwayFromCamera = new MatrixTransform3D(cameraRotationMatrix.Inverse());

            transform.Children.Add(model.Transform);
            transform.Children.Add(rotateAwayFromCamera);
            transform.Children.Add(translateFromCameraToOrigin);
            transform.Children.Add(translateFromOriginToHitPoint);
            transform.Children.Add(translateFromHitPointToEdge);
        ...
        /// <summary>
        /// Handler for interaction mode camera transform changed event
        /// attaches object to mouse cursor whenever camera is being moved
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        void InteractionModeCameraTransformChanged(object sender, EventArgs e)
        {
            AttachObjectToMouse(SelectedObject);
            /*Transform3DGroup transform = new Transform3DGroup();

            Matrix3D currentCameraRotationMatrix = GetCurrentCameraMatrix();

            if (!previousCameraRotationMatrix.IsIdentity)
            {
                TranslateTransform3D translateFromCameraToOrigin =
                new TranslateTransform3D(virtualCamera.WorldToCameraMatrix3D.OffsetX * -1,
                    virtualCamera.WorldToCameraMatrix3D.OffsetY * -1,
                    (virtualCamera.WorldToCameraMatrix3D.OffsetZ * -1));

                TranslateTransform3D translateFromOriginToCamera =
                    new TranslateTransform3D(virtualCamera.WorldToCameraMatrix3D.OffsetX,
                        virtualCamera.WorldToCameraMatrix3D.OffsetY,
                        virtualCamera.WorldToCameraMatrix3D.OffsetZ);
                MatrixTransform3D rotateAwayFromCamera = new MatrixTransform3D(previousCameraRotationMatrix.Inverse());
                MatrixTransform3D rotateToNewCamera = new MatrixTransform3D(currentCameraRotationMatrix);
                transform.Children.Add(SelectedObject.Transform);
                transform.Children.Add(translateFromCameraToOrigin);
                transform.Children.Add(rotateAwayFromCamera);
                transform.Children.Add(rotateToNewCamera);
                transform.Children.Add(translateFromOriginToCamera);
                SelectedObject.Transform = transform;
            }

            previousCameraRotationMatrix = currentCameraRotationMatrix;*/
        }

        /// <summary>
        /// Handler for mouse move event
        /// attaches object to mouse cursor whenever cursor is being moved
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        ...
        /// <summary>
        /// Attaches model to the cursor position
        /// </summary>
        /// <param name="model">Model</param>
        private void AttachObjectToMouse(GeometryModel3D model)
        {
            //get cursor position relative to HelixViewport
            Point p = Mouse.GetPosition(GraphicsViewport);

            //raycast into viewport to get cursor position and direction of ray
            var pointInMiddle = new Point3D();
            var pointInFar = new Point3D();
            Viewport3DHelper.Point2DtoPoint3D(GraphicsViewport.Viewport, p, out pointInMiddle, out pointInFar);
            ...
            TranslateTransform3D translateToOrigin =
                    new TranslateTransform3D((centerPoint.X * -1), (centerPoint.Y * -1), (centerPoint.Z * -1));// - 0.6);

            TranslateTransform3D translateFromOriginToCursor =
                new TranslateTransform3D(newPoint.X,
                    newPoint.Y,
                    newPoint.Z);
            ...
            transform.Children.Add(translateToOrigin);
            transform.Children.Add(translateFromOriginToCursor);
        ...
        /// handles raycast at mouse position and transforms object to specified position in world space
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        public new void MouseUp(object sender, MouseButtonEventArgs e)
        {
            //only used if grab carry is activated and there is an object selected
            if (SelectedObject != null && grabCarry)
            {
                //restore selected object material
                SelectedObject.Material = selectedObjectStoredMaterial;
                SelectedObject.BackMaterial = selectedObjectStoredMaterial;

                //unregister event handlers so that object can be detached from cursor
                virtualCamera.CameraTransformationChanged -= InteractionModeCameraTransformChanged;
                CompositeHost.MouseMove -= OnMouseMove;

                //if SPACE is pressed while mouse up the selected object will be returned to initial position
                bool backToInit = (Keyboard.IsKeyDown(Key.Space));

                //get mouse position
                Point p = e.GetPosition(GraphicsViewport);

                Point3D hitP;

                //hittest into space at mouse position
                GeometryModel3D source = FindHitSource(p, out hitP);
                if (source != null)
                {
                    if (backToInit)
                ...
                {
                    //if user hits empty space, transform back to previous position
                    SelectedObject.Transform = SelectedObjectStoredTransforms[SelectedObject];
                }

                //unset variables so that a new object can be selected
                selectedObjectStoredMaterial = null;
                SelectedObject = null;
                mouseIsDown = false;
            }
        ...
        /// <summary>
        /// Find the model hit at a specified position
        /// </summary>
        /// <param name="p">Specified position in screen coordinates</param>
        /// <param name="position">Hit position in world coordinates</param>
        GeometryModel3D FindHitSource(Point p, out Point3D position)
        {
            var hits = Viewport3DHelper.FindHits(GraphicsViewport.Viewport, p);

            foreach (var h in hits)
            {
                //if the selectedobject is hit, ignore it (we want to know the model behind it)
                if (h.Model == SelectedObject)
                    continue;
                position = h.Position;
                Console.WriteLine("Hit position: " + h.Position + ", hit distance: " + h.Distance + ", " + h);
                return h.Model as GeometryModel3D;
            }
        ...
        /// rotates and scales a selected object
        /// for testing purposes without touchscreen, in a finished application there would be touch 
        /// event handlers for this, but of course you need a touch screen for that
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event argument</param>
        public void OnMouseWheel(object sender, MouseWheelEventArgs e)
        {
            ...
            {
                //SPACE key used to distinguish between rotation and scale
                bool spaceKey = (Keyboard.IsKeyDown(Key.Space));

                Point3D centerPoint = GetCenterPoint(SelectedObject);

                TranslateTransform3D translateToOrigin =
                    new TranslateTransform3D(centerPoint.X * -1, centerPoint.Y * -1, centerPoint.Z * -1);

                TranslateTransform3D translateBack =
                    new TranslateTransform3D(centerPoint.X, centerPoint.Y, centerPoint.Z);

                Transform3DGroup group = new Transform3DGroup();

                //dont forget previous transforms
                group.Children.Add(SelectedObject.Transform);

                //translate to origin to allow for rotations and scale 'around' the objects own center
                group.Children.Add(translateToOrigin);
                ...
                if (spaceKey)
                {
                    ...
                    if (e.Delta > 0)
                    {
                        matrix = CalculateRotationMatrix(0, 0, 20);
                    }
                    else if (e.Delta < 0)
                    {
                        matrix = CalculateRotationMatrix(20, 0, 0);
                    }
                    MatrixTransform3D rotateMatrix = new MatrixTransform3D(matrix);
                    group.Children.Add(rotateMatrix);
                }
                else if (!spaceKey)
                {
                    ...
                    if (e.Delta > 0)
                    {
                        scaleTransform = new ScaleTransform3D(0.9, 0.9, 0.9);
                    }
                    else if (e.Delta < 0)
                    {
                        scaleTransform = new ScaleTransform3D(1.1, 1.1, 1.1);
                    }
                    group.Children.Add(scaleTransform);
                }

                //translate back to real position in space after transformation
                group.Children.Add(translateBack);

                SelectedObject.Transform = group;
            }
        }
        #endregion
        ...
        /// <summary>
        /// Calculate a rotation matrix in x, y and z direction
        /// </summary>
        ...
        /// <summary>
        /// Get the center point of a model
        /// </summary>
        /// <param name="model">Model</param>
        Point3D GetCenterPoint(Model3D model)
        {
            Rect3D bounds = model.Bounds;
            Point3D centerPoint = new Point3D(bounds.X + bounds.SizeX / 2, bounds.Y + bounds.SizeY / 2, bounds.Z + bounds.SizeZ / 2);
            return centerPoint;
        }

        /// <summary>
        /// Get the point inside a model that has the greatest distance to the camera position
        /// used to avoid objects being placed inside other objects, but rather in front of them
        /// </summary>
        /// <param name="model">Model</param>
        Point3D GetPointOfGreatestDistance(GeometryModel3D model)
        {
            ...
            //calculate the point with the greatest distance to the current camera position
            int index = 0;
            double distance = 0;
            int position = 0;
            foreach (Point3D point in (model.Geometry as MeshGeometry3D).Positions)
            {
                Vector3D tmp = new Vector3D(point.X, point.Y, point.Z);
                Point3D tmpPoint = model.Transform.Transform(point);
                double tmpDistance = tmpPoint.DistanceToSquared(cameraPoint);
        ...
        /// <summary>
        /// Get the current camera rotation matrix
        /// ignore the 4th row as it stores the cameras translation
        /// </summary>
        Matrix3D GetCurrentCameraMatrix()
        {
            Matrix3D cameraRotationMatrix = new Matrix3D();
            cameraRotationMatrix.M11 = virtualCamera.WorldToCameraMatrix3D.M11;
            cameraRotationMatrix.M12 = virtualCamera.WorldToCameraMatrix3D.M12;
            cameraRotationMatrix.M13 = virtualCamera.WorldToCameraMatrix3D.M13;
            cameraRotationMatrix.M14 = virtualCamera.WorldToCameraMatrix3D.M14;
            cameraRotationMatrix.M21 = virtualCamera.WorldToCameraMatrix3D.M21;
            cameraRotationMatrix.M22 = virtualCamera.WorldToCameraMatrix3D.M22;
            cameraRotationMatrix.M23 = virtualCamera.WorldToCameraMatrix3D.M23;
            cameraRotationMatrix.M24 = virtualCamera.WorldToCameraMatrix3D.M24;
            cameraRotationMatrix.M31 = virtualCamera.WorldToCameraMatrix3D.M31;
            cameraRotationMatrix.M32 = virtualCamera.WorldToCameraMatrix3D.M32;
            cameraRotationMatrix.M33 = virtualCamera.WorldToCameraMatrix3D.M33;
            cameraRotationMatrix.M34 = virtualCamera.WorldToCameraMatrix3D.M34;
        ...
        /// </summary>
        /// <param name="model">Rotation marix</param>
        Quaternion ConvertToQuaternion(Matrix3D matrix)
        {
            Quaternion quaternion = new Quaternion();
            float tr = (float)(matrix.M11 + matrix.M22 + matrix.M33);

            if (tr > 0)
            {
                float S = (float)(Math.Sqrt(tr + 1.0) * 2); // S=4*qw 
                quaternion.W = 0.25 * S;
                quaternion.X = matrix.M32 - matrix.M23 / S;
                quaternion.Y = (matrix.M13 - matrix.M31) / S;
                quaternion.Z = (matrix.M21 - matrix.M12) / S;
            }
            else if ((matrix.M11 > matrix.M22) && (matrix.M11 > matrix.M33))
            {
                float S = (float)(Math.Sqrt(1.0 + matrix.M11 - matrix.M22 - matrix.M33) * 2);
                quaternion.W = (matrix.M32 - matrix.M23) / S;
                quaternion.X = 0.25 * S;
            ...
            else if (matrix.M22 > matrix.M33)
            {
                ...
                quaternion.W = (matrix.M13 - matrix.M31) / S;
                quaternion.X = (matrix.M12 + matrix.M21) / S;
                quaternion.Y = 0.25 * S;
                quaternion.Z = (matrix.M23 + matrix.M32) / S;
            }
            else
            {
                float S = (float)(Math.Sqrt(1.0 + matrix.M33 - matrix.M11 - matrix.M22) * 2); // S=4*qz
                quaternion.W = (matrix.M21 - matrix.M12) / S;
                quaternion.X = (matrix.M13 - matrix.M31) / S;
                quaternion.Y = (matrix.M23 - matrix.M32) / S;
                quaternion.Z = 0.25 * S;
            }
            return quaternion;
        }
    ...
    #region unchanged DepthToTextConverter class
    /// <summary>
